...
final class ProxyAPIClient: @unchecked Sendable {
    ...
    private init() {
        // Use centralized AppEnvironment for consistent base URL across all services
                            ...
                            if let data = jsonString.data(using: .utf8),
                               let chunk = try? JSONDecoder().decode(StreamChunk.self, from: data) {
                                ...
                                if let provider = chunk.provider {
                                    ...
                                    let effortInfo = chunk.effort ?? "unknown"
                                    print("[ProxyAPI] ✅ effort=\(effortInfo) provider=\(provider)")
                                }
